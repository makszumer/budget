  ...
  return (
    <div className="w-72 bg-white dark:bg-gray-900 border-r border-gray-200 dark:border-gray-800 h-screen overflow-y-auto p-4 flex-shrink-0">
      ...
      <nav className="space-y-2">
        <MenuItem icon={LayoutDashboard} label="Dashboard" page="dashboard" active={currentPage === "dashboard"} />
        {isAdmin && <MenuItem icon={Shield} label="Admin Panel" page="admin" active={currentPage === "admin"} />}
        <MenuItem icon={Repeat} label="Standing Orders" page="recurring" active={currentPage === "recurring"} />
        <MenuItem icon={Wallet} label="Budget Envelopes" page="budget-envelopes" active={currentPage === "budget-envelopes"} />
        <MenuItem icon={Tag} label="Custom Categories" page="custom-categories" active={currentPage === "custom-categories"} />
        <MenuItem icon={MessageSquare} label="AI Assistant" page="ai-assistant" active={currentPage === "ai-assistant"} isPremiumFeature={true} />
        ...
        <div className="space-y-1">
          ...
          >
            ...
            {investingOpen ? <ChevronDown className="h-4 w-4" /> : <ChevronRight className="h-4 w-4" />}
          </button>
          {investingOpen && (
            <div className="space-y-1 mt-1">
              <SubMenuItem label="Overview" page="investing-overview" active={currentPage === "investing-overview"} />
              <SubMenuItem label="Crypto" page="investing-crypto" active={currentPage === "investing-crypto"} />
              <SubMenuItem label="Stocks" page="investing-stocks" active={currentPage === "investing-stocks"} />
              <SubMenuItem label="Bonds" page="investing-bonds" active={currentPage === "investing-bonds"} />
              <SubMenuItem label="ETFs" page="investing-etfs" active={currentPage === "investing-etfs"} />
              <SubMenuItem label="Inflation" page="investing-inflation" active={currentPage === "investing-inflation"} />
            </div>
          )}
        </div>

        <MenuItem icon={Calculator} label="Compound Calculator" page="calculator" active={currentPage === "calculator"} />
          ...
          <div className="flex items-center justify-between gap-1 bg-gray-100 dark:bg-gray-800 p-1 rounded-lg">
            <button
              ...
              className={`flex-1 flex items-center justify-center gap-1.5 py-2 px-2 rounded-md text-xs font-medium transition-all ${
                theme === 'light' && !isDark ? 'bg-white dark:bg-gray-700 shadow text-gray-900 dark:text-white' : 'text-gray-600 dark:text-gray-400 hover:text-gray-900 dark:hover:text-white'
              }`}
            ...
            <button
              ...
              className={`flex-1 flex items-center justify-center gap-1.5 py-2 px-2 rounded-md text-xs font-medium transition-all ${
                isDark ? 'bg-white dark:bg-gray-700 shadow text-gray-900 dark:text-white' : 'text-gray-600 dark:text-gray-400 hover:text-gray-900 dark:hover:text-white'
              }`}
        ...
        <div className="px-4 py-3">
          <p className="text-xs font-medium text-gray-500 dark:text-gray-400 mb-3">Default Currency</p>
