...
// RevenueCat API key (iOS only)
const REVENUECAT_IOS_KEY = 'appl_tTLUAIDqZYxMKuordyfuFhTREVy'; // TODO: replace after RC setup
...
export const PricingPage = ({ onGoBack }) => {
  ...
  const handleIOSPurchase = async (productId) => {
    ...
    try {
      ...
      if (!packageToBuy) {
        throw new Error(`Product ${productId} not found. Make sure it is approved in App Store Connect.`);
      }

      // Trigger Apple purchase sheet
      const { customerInfo, transaction } = await Purchases.purchasePackage({
        aPackage: packageToBuy
      });

      // Verify with our backend and grant premium
      await axios.post(
  ...
  // Route to correct handler based on platform
  const handleSubscribe = (packageId, isDiscounted = false) => {
    if (isNativeIOS()) {
      ...
      if (productId) {
        ...
        toast.error('This offer is not available in the App Store. Visit our website for special pricing.');
      }
    } else {
  ...
  // ── Trial (no payment, same on all platforms) ──
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-50 to-slate-100 dark:from-gray-900 dark:to-gray-950 py-12 px-4">
      <div className="max-w-6xl mx-auto">
        ...
        {/* Discount offer — web only (Apple doesn't allow discounts outside IAP) */}
            ...
            <div className="flex items-start gap-4">
              ...
              <div className="flex-1">
                ...
                <div className="flex gap-3">
                  <Button
                    onClick={() => handleStripeSubscribe('monthly_discount', true)}
                  ...
                  <Button
                    onClick={() => handleStripeSubscribe('yearly_discount', true)}
          ...
          <Card className="relative border-2 border-amber-400 dark:border-amber-500 shadow-xl bg-white dark:bg-gray-800">
            ...
            <CardContent>
              ...
              <div className="space-y-3">
                {/* Free Trial — same on all platforms */}
